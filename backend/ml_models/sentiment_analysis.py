"""Sentiment analysis module using FinBERT NLP for stock news scoring and asset grading."""
...
def analyze_news_sentiment(ticker):
    ...
    try:
        ...
        texts_to_analyze = [
            f"{headline}. {summary_text}"[:1000] if summary_text else headline 
            for headline, summary_text in zip(headlines, summaries)
        ]
        ...
        for headline, link, summary_text, publisher, result in zip(headlines, links, summaries, publishers, results):
...
def calculate_asset_grade(price_forecasts, div_forecasts, sentiment_score, info, is_fund=False):
    ...
    logger.debug("Raw asset grade score: %d", score)
    ...
    fundamentals_data = {}
    if pos_factors:
        fundamentals_data["positive"] = pos_factors
    if neg_factors:
        fundamentals_data["negative"] = neg_factors
